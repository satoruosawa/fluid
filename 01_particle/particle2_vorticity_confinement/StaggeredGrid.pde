class StaggeredGrid {
  ...
  public StaggeredGrid(int gridWidth, int numGridX, int numGridY) {
    ...
    vorticities = new float[numGridX - 1][numGridY - 1];
    for (int j = 0; j < numGridY - 1; j++) {
      for (int i = 0; i < numGridX - 1; i++) {
        vorticities[i][j] = 0.0;
      }
  ...
  protected void updateVorticity() {
    // BUG: Check algorithm
    float coef = 0.01;
    for (int j = 0; j < numGridY - 1; j++) {
      for (int i = 0; i < numGridX - 1; i++) {
        vorticities[i][j] =
          -prevVelocitiesX[i + 1][j] + prevVelocitiesY[i][j + 1] +
          prevVelocitiesX[i + 1][j + 1] - prevVelocitiesY[i + 1][j + 1];
      }
    }
    for (int j = 1; j < numGridY - 1; j++) {
      for (int i = 1; i < numGridX - 1; i++) {
        float topLeftVorticity = vorticities[i - 1][j - 1];
        float topRightVorticity = vorticities[i][j - 1];
        float bottomLeftVorticity = vorticities[i - 1][j];
        float bottomRightVorticity = vorticities[i][j];
        PVector n = new PVector(
          (topRightVorticity + bottomRightVorticity -
            topLeftVorticity - bottomLeftVorticity) / 2.0,
          (bottomLeftVorticity + bottomRightVorticity -
            topLeftVorticity - topRightVorticity) / 2.0
        ).normalize();
        PVector force = n.mult(coef);
        addVelocityX(i - 0.5, j, force.x);
        addVelocityX(i + 0.5, j, force.x);
        addVelocityY(i, j - 0.5, force.y);
        addVelocityY(i, j + 0.5, force.y);
      }
    }
    copyVelocitiesToPrevVelocities();
